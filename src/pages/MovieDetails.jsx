  ...
  if (error) {
    return (
      <div className="h-screen grid place-items-center">
        <div className="max-w-3xl w-11/12 text-center">
          ...
          <Link className="inline-block mt-6 underline cursor-pointer" to="/">
            Go Home
          </Link>
        </div>
      </div>
    ...
    <>
      <article className="flex flex-col min-h-screen sm:flex-row">
        ...
        <div className="text-[#999] sm:max-h-screen sm:overflow-y-auto sm:self-center sm:flex-1 px-4 sm:px-8 md:px-14 py-6 sm:py-20">
          <h2 className="text-white text-3xl sm:text-4xl font-semibold leading-none">
          ...
          {movie.genres && (
            <div className="flex flex-wrap gap-1 items-center mt-4">
              {movie.genres.map((genre, i) => (
                <span
                  key={i}
                  className="text-sm font-medium bg-black text-white py-0.5 px-3 rounded-full"
                >
                  {genre.name}
                </span>
              ))}
